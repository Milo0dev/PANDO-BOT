...
module.exports = {
  ...
  async execute(message, client) {
    ...
    if (ticket) {
      ...
      if (s.log_deletes && s.log_channel) {
        const logCh = guild.channels.cache.get(s.log_channel);
        if (logCh) {
          ticketLogs.add(guild.id, message.channel.id, "delete", {
    ...
    if (!ml.enabled || !ml.log_msg_delete || !ml.channel) return;
    if (ml.channel === settings.get(guild.id).log_channel && ticket) return; // evitar doble log
